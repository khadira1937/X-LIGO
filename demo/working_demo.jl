...
Clean, working demo with smart AI chat
"""
...
function smart_ai_response(message::String, user_context::Dict)
...
function simulate_attack()
    ...
    @info "Simulating attack scenario" attack_type="flash_loan" user_id=demo_state.user_id
    @info "Security incident recorded" user_id=demo_state.user_id severity="CRITICAL" reason="Health factor 1.05 below threshold 1.2"
            ...
            HTTP.serve("0.0.0.0", 3000) do request::HTTP.Request
                if request.method == "POST" && request.target == "/chat"
                    try
                        ...
                        ai_response = smart_ai_response(message, user_context)
...
function interactive_chat()
    ...
    while true
        ...
        try
            print("🤖 AI: ")
            sleep(0.5)  # Brief thinking pause
